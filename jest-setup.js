...
// Factory, not a shared singleton: each getContext('2d') call returns a fresh
// object so canvas state (fillStyle, shadowBlur, ...) cannot leak between
// canvases or between tests.
function createMockContext2D() {
  return {
    ...
    measureText: jest.fn((text) => ({ width: Math.max(1, String(text).length * 10) })),
    fillText: jest.fn(),
    strokeText: jest.fn(),
    drawImage: jest.fn(),
