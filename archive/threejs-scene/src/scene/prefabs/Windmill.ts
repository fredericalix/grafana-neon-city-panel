...
/**
 * Windmill Prefab - Cyberpunk/Tron Style Energy Turbine
 * Features: Rotating energy blades, neon rings, plasma core, energy particles
 */
export class WindmillPrefab extends BasePrefab {
  ...
  private createBasePlatform(): void {
    // Hexagonal base platform (scaled 1.5x)
    ...
    const ringGeo = new THREE.TorusGeometry(0.50, 0.02, 4, 12);
  ...
  private createTurbineTower(): void {
    // Main tower body - sleek tapered cylinder (scaled 1.5x)
  ...
  private addTowerNeonEdges(): void {
    ...
    // Vertical edge lines (scaled 1.5x)
    for (let i = 0; i < 4; i++) {
      const angle = (i / 4) * Math.PI * 2 + Math.PI / 8;
      const topRadius = 0.18;  // scaled 1.5x
      const bottomRadius = 0.33;  // scaled 1.5x
  ...
  private createNeonRings(): void {
    // Multiple glowing rings along the tower (scaled 1.5x)
    const heights = [0.3, 0.6, 0.9];  // scaled 1.5x
    const colors = [COLORS.glow.cyan, COLORS.glow.magenta, COLORS.glow.cyan];

    for (let i = 0; i < heights.length; i++) {
      const y = heights[i];
      const radius = 0.33 - (y / 1.05) * 0.15;  // scaled 1.5x

      const ringGeo = new THREE.TorusGeometry(radius + 0.03, 0.018, 4, 16);  // scaled 1.5x
      const ringMat = new THREE.MeshBasicMaterial({
        color: colors[i],
        transparent: true,
        opacity: 0.8,
  ...
  private createEnergyBlades(): void {
    ...
    // Central hub - glowing energy core
    ...
    const hubGeo = new THREE.CylinderGeometry(0.12, 0.12, 0.12, 12);  // scaled 1.5x
    const hub = new THREE.Mesh(hubGeo, hubMat);
    hub.rotation.x = Math.PI / 2;
    this.blades.add(hub);

    // Hub glow ring (scaled 1.5x)
    ...
    hubRing.position.z = 0.06;  // scaled 1.5x
    this.addGlowMesh(hubRing);
    this.blades.add(hubRing);

    // Create 3 energy blades (more futuristic than 4)
    const bladeCount = 3;
    ...
    // Position blade assembly (scaled 1.5x)
  ...
  private createEnergyBlade(): THREE.Group {
    ...
    const armGeo = new THREE.BoxGeometry(0.525, 0.0375, 0.0225);  // scaled 1.5x
    const arm = new THREE.Mesh(armGeo, armMat);
    arm.position.x = 0.27;  // scaled 1.5x
    bladeGroup.add(arm);

    // Energy blade panel (glowing) - scaled 1.5x
    ...
    const panelGeo = new THREE.PlaneGeometry(0.42, 0.12);  // scaled 1.5x
    const panel = new THREE.Mesh(panelGeo, panelMat);
    panel.position.set(0.3, 0, 0.015);  // scaled 1.5x
    panel.rotation.x = Math.PI * 0.05;
    this.addGlowMesh(panel);
    bladeGroup.add(panel);

    // Blade tip glow (scaled 1.5x)
    ...
    tip.position.x = 0.54;  // scaled 1.5x
    this.addGlowMesh(tip);
    bladeGroup.add(tip);

    // Neon edge on blade (scaled 1.5x)
    const edgePoints = [
      new THREE.Vector3(0.03, 0, 0.03),
      new THREE.Vector3(0.525, 0, 0.03),
    ];
  ...
  private createEnergyCore(): void {
    // Glowing energy core in the center of the tower (scaled 1.5x)
    ...
    this.energyCore.position.set(0, 0.85, 0.20);  // scaled 1.5x
    this.addGlowMesh(this.energyCore);
    this.group.add(this.energyCore);

    // Core outer ring (scaled 1.5x)
    ...
    outer.position.set(0, 0.85, 0.18);  // scaled 1.5x
  ...
  private createEnergyParticles(): void {
    // Energy particles around the turbine (replaces fire effect)
    ...
    for (let i = 0; i < particleCount; i++) {
      ...
      const radius = 0.3 + Math.random() * 0.225;  // scaled 1.5x
      positions[i * 3] = Math.cos(angle) * radius;
      positions[i * 3 + 1] = 0.45 + Math.random() * 0.75;  // scaled 1.5x
      positions[i * 3 + 2] = Math.sin(angle) * radius;

      // Cyan/magenta energy colors
  ...
  protected onStatusChange(status: BuildingStatus): void {
    if (!this.body || !this.energyParticles) return;

    const isOffline = status === 'offline';
    const isWarning = status === 'warning';
    const isCritical = status === 'critical';

    // Dim neon rings when offline
  ...
  override update(deltaTime: number): void {
    ...
    if (this.energyParticles?.visible) {
      ...
      for (let i = 0; i < positions.count; i++) {
        ...
        y += deltaTime * speed * 0.45;  // scaled 1.5x
        if (y > 1.35) {  // scaled 1.5x
          y = 0.45;  // scaled 1.5x
        }
