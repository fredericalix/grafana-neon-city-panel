...
const log = createLogger('TrafficManager');
...
export class TrafficManager {
   ...
   */
  setRoads(roads: string[]): void {
    this.roads = roads;
    this.pathGenerator.parseRoads(roads);

    log.log(`[TrafficManager] Roads set with ${this.pathGenerator.getRoadCellCount()} cells`);
  ...
  setDensity(density: number): void {
    ...
    log.log(`[TrafficManager] Density set to ${this.config.density}%`);
  }
  ...
  setSpeed(speed: TrafficSpeed): void {
    ...
    log.log(`[TrafficManager] Speed set to ${speed}`);
  }
  ...
  setEnabled(enabled: boolean): void {
    ...
    log.log(`[TrafficManager] ${enabled ? 'Enabled' : 'Disabled'}`);
  ...
  update(deltaTime: number): void {
    ...
    if (vehicleDeficit > 0) {
      ...
      // Too many vehicles - remove one (oldest first, not random)
  ...
  private spawnVehicle(): void {
    ...
    if (path.length < 3) {
      log.log('[TrafficManager] Generated path too short, skipping spawn');
      vehicle.dispose();
      return;
    }
    ...
    log.log(`[TrafficManager] Spawned ${vehicle.type} (${this.vehicles.length} vehicles)`);
  }
  ...
  private handleInactiveVehicles(): void {
    ...
    for (const vehicle of this.vehicles) {
      ...
      if (vehicle.needsNewPath) {
        ...
        // Only dispose if truly inactive (shouldn't happen normally now)
        vehicle.startDisposal();
        this.disposingVehicles.push(vehicle);
      }
    }
  ...
  private removeOldestVehicle(): void {
    ...
    if (vehicle) {
      vehicle.startDisposal();
      this.disposingVehicles.push(vehicle);
      log.log(`[TrafficManager] Removed oldest vehicle (${this.vehicles.length} remaining)`);
    }
  }
  ...
  clearAllVehicles(): void {
    ...
    log.log('[TrafficManager] Cleared all vehicles');
  }
...
// Export all traffic components for convenience
